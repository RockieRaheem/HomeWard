...
export default function HomeDashboard({ onNavigate, onNavigateGoal, onSignOut, user }: { onNavigate: (route: AppScreen) => void; onNavigateGoal?: (id: string) => void; onSignOut?: () => void; user?: { id: string; name: string; phone: string } }) {
  ...
  const confirmSignOut = () => Alert.alert('Sign out?', 'You will need your phone number and PIN to return.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Sign out', style: 'destructive', onPress: onSignOut }]);
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchData(); }} tintColor={Colors.primary} />}>
        <View style={styles.topBar}>
          ...
          <View style={styles.topActions}>
            ...
            <TouchableOpacity style={styles.signOutButton} onPress={confirmSignOut} accessibilityLabel="Sign out"><MaterialCommunityIcons name="logout-variant" size={17} color={Colors.primary} /><Text style={styles.signOutText}>Sign out</Text></TouchableOpacity><TouchableOpacity style={styles.avatar} onPress={confirmSignOut} accessibilityLabel="Account options"><Text style={styles.avatarText}>{initials(user?.name)}</Text></TouchableOpacity>
          </View>
        </View>
