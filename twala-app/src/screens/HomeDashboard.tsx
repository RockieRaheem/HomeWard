...
export default function HomeDashboard({ onNavigate, onNavigateGoal, onSignOut, onProfileUpdate, user }: { onNavigate: (route: AppScreen) => void; onNavigateGoal?: (id: string) => void; onSignOut?: () => void; onProfileUpdate?: (name: string) => void; user?: { id: string; name: string; phone: string } }) {
  ...
  const confirmSignOut = () => { setProfileMenuOpen(false); setSignOutConfirmOpen(true); };
  const completeSignOut = () => { setSignOutConfirmOpen(false); onSignOut?.(); };
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={signOutConfirmOpen} transparent animationType="fade" onRequestClose={() => setSignOutConfirmOpen(false)}><View style={styles.editOverlay}><View style={styles.signOutSheet}><View style={styles.signOutIcon}><MaterialCommunityIcons name="logout-variant" size={25} color={Colors.error} /></View><Text style={styles.editTitle}>Sign out of HomeWard?</Text><Text style={styles.editSubtitle}>Your private account will be closed on this device. You can sign in again with your phone number and PIN.</Text><View style={styles.editActions}><TouchableOpacity style={styles.editCancel} onPress={() => setSignOutConfirmOpen(false)}><Text style={styles.editCancelText}>Stay signed in</Text></TouchableOpacity><TouchableOpacity style={styles.signOutConfirmButton} onPress={completeSignOut}><Text style={styles.editSaveText}>Sign out</Text></TouchableOpacity></View></View></View></Modal>
    </View>
  );
}
