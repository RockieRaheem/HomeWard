        ...
        <View style={styles.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.details}>
              ...
              {tx.recipientCommitment && tx.safetyAuditHash ? <View style={styles.privacyProof}><MaterialCommunityIcons name="shield-lock-outline" size={18} color={Colors.primary} /><View style={{ flex: 1 }}><Text style={styles.privacyProofTitle}>{t('Private recipient proof')}</Text><Text style={styles.privacyProofText}>{t('Stellar contains a non-reversible commitment, never the recipient name or phone.')}</Text><Text style={styles.privacyProofHash}>RC {tx.recipientCommitment.slice(0, 24)} · SA {tx.safetyAuditHash.slice(0, 24)}</Text></View></View> : null}
            </View>
...
const styles = StyleSheet.create({
  ...
  privacyProof: { flexDirection: 'row', gap: 9, backgroundColor: Colors.surfaceContainerLow, borderRadius: BorderRadius.lg, padding: 11, borderWidth: 1, borderColor: Colors.primary + '33' }, privacyProofTitle: { fontSize: Typography.bodySm.fontSize, fontFamily: 'Inter', fontWeight: '800', color: Colors.onSurface }, privacyProofText: { fontSize: 11, fontFamily: 'Inter', color: Colors.onSurfaceVariant, marginTop: 2, lineHeight: 16 }, privacyProofHash: { fontSize: 10, fontFamily: 'Inter', fontWeight: '700', color: Colors.primary, marginTop: 5 },
});
