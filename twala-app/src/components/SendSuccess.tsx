      ...
      <View style={styles.overlay}>
        <Animated.View style={[styles.card, { transform: [{ scale }] }]}>
          ...
          <View style={styles.checkSection}>
            ...
            <Text style={styles.title}>{t('Transfer Successful!')}</Text>
            <Text style={styles.subtitle}>{t('Money is on its way to')} {recipientName}</Text>
          </View>

          <Animated.View style={[styles.detailsSection, { opacity: detailsOpacity }]}>
            <View style={styles.amountRow}>
              <Text style={styles.sentLabel}>{t('You Sent')}</Text>
              <Text style={styles.sentAmount}>${amountUsdc.toFixed(2)} USDC</Text>
            </View>

            <View style={styles.amountRow}>
              <Text style={styles.sentLabel}>{t('Recipient Gets')}</Text>
            ...
            {recipientRelationship ? <View style={styles.detailRow}><MaterialCommunityIcons name="account-heart-outline" size={16} color={Colors.onSurfaceVariant} /><Text style={styles.detailLabel}>{t('Relationship')}</Text><Text style={styles.detailValue}>{recipientRelationship}</Text></View> : null}

            {purpose ? <View style={styles.detailRow}><MaterialCommunityIcons name="target" size={16} color={Colors.onSurfaceVariant} /><Text style={styles.detailLabel}>{t('Purpose')}</Text><Text style={styles.detailValue}>{purpose}</Text></View> : null}
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailLabel}>{t('New Balance')}</Text>
            ...
            {goalTitle ? (
              <View style={styles.goalImpact}><MaterialCommunityIcons name="flag-checkered" size={20} color={Colors.primary} /><View style={{ flex: 1 }}><Text style={styles.goalImpactLabel}>{t('FAMILY OUTCOME UPDATED')}</Text><Text style={styles.goalImpactTitle}>{goalTitle}</Text><Text style={styles.goalImpactText}>{t('This transfer has been linked to the family goal.')}</Text></View></View>
            ) : null}

            {confirmedAt ? <View style={styles.consentNote}><MaterialCommunityIcons name="shield-check-outline" size={15} color={Colors.primary} /><Text style={styles.consentText}>{t('Confirmed by you')} {new Date(confirmedAt).toLocaleString('en-UG', { timeZone: 'Africa/Kampala', dateStyle: 'medium', timeStyle: 'short' })}</Text></View> : null}
            ...
            {stellarTxHash && stellarExplorerUrl ? (
              ...
              >
                ...
                <View style={styles.proofText}>
                  <Text style={styles.proofTitle}>{t('Verified on Stellar Testnet')}</Text>
